...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--history-file",
        help="Path to history file to use (for resuming, specify the same history file from previous run)",
        default=None,
    )
...
async def main_async() -> int:
    ...
    try:
        ...
        if latest_checkpoint:
            ...
            history_file = openevolve.history_logger.file_path if hasattr(openevolve, 'history_logger') and openevolve.history_logger else None
            if history_file:
                print(f"To resume, use: --checkpoint {latest_checkpoint} --history-file {history_file}")
            else:
