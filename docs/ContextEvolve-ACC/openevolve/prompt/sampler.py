...
def format_abstract(abstract: Optional[str]) -> str:
    """
    Format abstract for display in prompts.
    If abstract is JSON format with Same/Diff fields, format it nicely.
    Otherwise, return as-is.
    
    Args:
        abstract: Abstract string (may be JSON or plain text)
    ...
    try:
        ...
        abstract_dict = json.loads(abstract)
        if isinstance(abstract_dict, dict) and "Same" in abstract_dict and "Diff" in abstract_dict:
            # Format JSON abstract
            same_items = abstract_dict.get("Same", [])
            diff_items = abstract_dict.get("Diff", [])
            
            formatted = []
            if same_items:
                formatted.append("**Same (shared characteristics):**")
                for item in same_items:
                    formatted.append(f"  - {item}")
            
            if diff_items:
                formatted.append("**Diff (changes in child):**")
                for item in diff_items:
                    formatted.append(f"  - {item}")
            
            if not formatted:
                return "No abstract details available"
...
class PromptSampler:
    ...
    def build_prompt(
        ...
        parent_gradient: Optional[str] = None,
        reference_programs: List[Dict[str, Any]] = [],
        reference_sources: Optional[Dict[str, List[Dict[str, Any]]]] = None,
        **kwargs: Any,
    ) -> Dict[str, str]:
        """
        ...
        # Format RL-Enhanced Context
        parent_gradient_str = parent_gradient or "No gradient provided."
        
        # Format reference programs: In RL mode, use reference_programs if available; otherwise use traditional evolution history
        if reference_programs and reference_sources:
            # RL mode: Use LLM-selected references, classified by source
            reference_programs_str = self._format_reference_programs_with_sources(
                reference_programs, reference_sources, language
            )
        else:
            # Non-RL mode or fallback: Use traditional evolution history
            reference_programs_str = self._format_evolution_history(
        ...
        user_message = user_template.format(
            ...
            reference_programs=reference_programs_str,
            current_program=current_program,
            language=language,
            artifacts=artifacts_section,
            parent_gradient=parent_gradient_str,
            **kwargs,
        )
    ...
    def _format_evolution_history(
        ...
        num_previous = min(self.config.num_previous_programs, len(previous_programs))
        selected_previous = previous_programs[-num_previous:] if num_previous > 0 else []
    ...
    def _format_inspirations_section(
        ...
        # Limit the number of inspirations based on config
        # Use num_inspiration_programs if available, otherwise fall back to num_inspirations
        num_inspirations = getattr(self.config, 'num_inspiration_programs', None) or getattr(self.config, 'num_inspirations', 5)
        num_inspirations = min(num_inspirations, len(inspirations))
        if num_inspirations <= 0:
            return ""
        
        selected_inspirations = inspirations[:num_inspirations]
        ...
        for i, program in enumerate(selected_inspirations):
            # Use the full program code
            program_code = program.get("code", "")
    ...
    def _format_reference_programs_with_sources(
        self,
        reference_programs: List[Dict[str, Any]],
        ...
        """
        Format reference programs by classifying them according to their source.
        
        Args:
            reference_programs: List of reference programs selected by LLM
            reference_sources: Dict mapping source names to program lists
            language: Programming language
        
        Returns:
            Formatted string with references classified by source
        """
        # Create ID to source mapping
        id_to_source = {}
        for source_name, programs in reference_sources.items():
            for program in programs:
                program_id = program.get("id") if isinstance(program, dict) else program.id
                id_to_source[program_id] = source_name
        
        # Classify references by source
        previous_refs = []
        top_refs = []
        inspiration_refs = []
        ...
        for ref in reference_programs:
            ref_id = ref.get("id")
            source = id_to_source.get(ref_id, "unknown")
            
            if source == "island_previous_programs":
                previous_refs.append(ref)
            elif source == "island_top_programs":
                top_refs.append(ref)
            elif source == "inspirations":
                inspiration_refs.append(ref)
            else:
                top_refs.append(ref)
        ...
        if previous_refs:
            previous_attempt_template = self.template_manager.get_template("previous_attempt")
            previous_attempts_str = ""
            for i, program in enumerate(reversed(previous_refs)):
                changes = program.get("changes", "Unknown changes")
                ...
                if numeric_improved and all(numeric_improved):
                    outcome = "Improvement in all metrics"
                elif numeric_improved and not any(numeric_improved):
                    outcome = "Regression in all metrics"
            ...
            result_parts.append("## Previous Attempts\n\n" + previous_attempts_str.strip())
        
        # Format top programs if any
        if top_refs:
            top_program_template = self.template_manager.get_template("top_program")
            top_programs_str = ""
            for i, program in enumerate(top_refs):
                program_code = program.get("code", "")
                score = safe_numeric_average(program.get("metrics", {}))
            ...
            result_parts.append("## Top Performing Programs\n\n" + top_programs_str.strip())
        
        # Format inspirations if any
        if inspiration_refs:
            inspirations_section_str = self._format_inspirations_section(inspiration_refs, language)
            if inspirations_section_str:
                result_parts.append(inspirations_section_str)
        
        if not result_parts:
            return "No reference programs selected."
    ...
    def _extract_unique_features(self, program: Dict[str, Any]) -> str:
        ...
        for metric_name, value in metrics.items():
            if isinstance(value, (int, float)):
                if value >= 0.9:
                    features.append(f"Excellent {metric_name} ({value:.4f})")
                elif value <= 0.3:
    ...
    def _format_history(self, history: List[Dict[str, Any]]) -> str:
        """
        Format history entries for prompt inclusion.
        
        Args:
            history: List of history entries, each containing parent_abstract, child_abstract, 
                    and either metrics_delta (new format) or fitness_delta (legacy format)
        ...
        for i, entry in enumerate(history, 1):
            parent_abstract = format_abstract(entry.get("parent_abstract"))
            child_abstract = format_abstract(entry.get("child_abstract"))
            
            # Handle both new format (metrics_delta) and legacy format (fitness_delta)
            metrics_delta = entry.get("metrics_delta")
            fitness_delta = entry.get("fitness_delta")
            
            # Check if this was a successful attempt (has_improvement field)
            has_improvement = entry.get("has_improvement", None)
            if has_improvement is True:
                outcome_label = "✅ Successful (at least one metric improved)"
            elif has_improvement is False:
                outcome_label = "❌ Failed (all metrics regressed or unchanged)"
            else:
                outcome_label = "⚠️ Unknown outcome"
            
            entry_str = f"**Evolution Example {i}:** {outcome_label}\n"
            entry_str += f"**Parent Abstract:** {parent_abstract}\n"
            entry_str += f"**Child Abstract:** {child_abstract}\n"
            
            if metrics_delta:
                # New format: show all metric deltas
                delta_parts = []
                
                # Helper function to format delta with proper sign handling
                def format_delta(delta: float) -> str:
                    """Format delta value, handling zero values correctly"""
                    # Use a small epsilon to detect near-zero values
                    epsilon = 1e-10
                    if abs(delta) < epsilon:
                        ...
                        return f"{delta:.4f}"  # Negative numbers already have '-' sign
                
                for metric_name, delta in sorted(metrics_delta.items()):
                    if isinstance(delta, (int, float)):
                        delta_parts.append(f"{metric_name}: {format_delta(float(delta))}")
                
                if delta_parts:
                    entry_str += f"**Metrics Delta:** {', '.join(delta_parts)}\n"
            elif fitness_delta is not None:
                # Legacy format: show fitness_delta
                # Helper function to format delta with proper sign handling
                def format_delta(delta: float) -> str:
                    """Format delta value, handling zero values correctly"""
                    epsilon = 1e-10
                ...
                entry_str += f"**Fitness Delta:** {format_delta(float(fitness_delta))}\n"
