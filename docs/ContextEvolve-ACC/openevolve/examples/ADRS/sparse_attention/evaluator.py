...
def run_benchmark_and_collect_metrics(sparse_attention_config):
    ...
    # Model configuration
    # Support local model path via environment variable or use default
    import os
    model_name: str = "/home/dataset-local/shy/lab/icml26/data/llama-3.1-8b-instruct"
...
def evaluate(program_path: str):
    # Convert program_path to absolute path before changing directory
    program_path = str(Path(program_path).absolute())
    
    # Get the directory where this evaluator.py file is located
    evaluator_dir = Path(__file__).parent.absolute()
    sparse_attention_hub_dir = evaluator_dir / "sparse-attention-hub"
    
    if not sparse_attention_hub_dir.exists():
        raise FileNotFoundError(
            f"sparse-attention-hub directory not found at {sparse_attention_hub_dir}. "
            f"Current working directory: {os.getcwd()}, Evaluator directory: {evaluator_dir}"
        )
    
    # Check if program_path exists before changing directory
    if not os.path.isfile(program_path):
        raise FileNotFoundError(f"No such file: {program_path}")
    
    old_path = os.getcwd()
    os.chdir(str(sparse_attention_hub_dir))
    sys.path.insert(0, str(sparse_attention_hub_dir))
