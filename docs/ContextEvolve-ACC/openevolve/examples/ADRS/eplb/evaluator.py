...
def simulate_inference(log2phy: torch.Tensor, logcnt: torch.Tensor, workload: torch.Tensor) -> float:
    ...
    for layer_id in range(num_layers):
        for logical_id in range(num_logical_experts):
            ...
            # 过滤掉无效的映射（-1 表示无效）
            valid_mask = physical_ids >= 0
            if not valid_mask.any():
                continue  # 如果没有有效的物理映射，跳过这个逻辑专家
            
            valid_physical_ids = physical_ids[valid_mask]
            num_valid_replicas = valid_mask.sum().item()

            # 计算每个副本的负载（基于有效副本数量）
            replica_load = logical_load / num_valid_replicas if num_valid_replicas > 0 else 0

            # 分配负载到有效的物理专家
            total_physical_load[layer_id, valid_physical_ids] += replica_load
