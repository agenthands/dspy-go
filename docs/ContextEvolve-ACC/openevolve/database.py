...
class ProgramDatabase:
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        # Cemetery of discarded programs, mapping program_id to its sample count
        self.cemetery: OrderedDict[str, int] = OrderedDict()
    ...
    def sample(self, n: Optional[int] = None) -> Tuple[Program, List[Program]]:
        """
        Sample a program and inspirations for the next evolution step

        Args:
            n: Number of inspirations to sample. If None, uses config.num_inspirations or default 5.
        ...
        # Select inspirations using configured number or provided parameter
        if n is None:
            num_inspirations = getattr(self.config, 'num_inspirations', 5)
        else:
            num_inspirations = n
    ...
    def add_to_cemetery(self, program: Program) -> None:
        """
        Add a discarded program to the cemetery.

        Args:
            program: The program to add to the cemetery.
        """
        cemetery_size = getattr(self.config, "cemetery_size", 100)
        if cemetery_size <= 0:
            return  # Cemetery is disabled

        # Add program with a sample count of 0.
        # If it already exists, move it to the end to mark it as recently added.
        if program.id in self.cemetery:
            del self.cemetery[program.id]
        
        self.cemetery[program.id] = 0

        # Enforce size limit by removing the oldest entries (FIFO)
        while len(self.cemetery) > cemetery_size:
            self.cemetery.popitem(last=False)

        logger.info(f"Added program {program.id} to cemetery (size: {len(self.cemetery)}/{cemetery_size})")

    def has_cemetery_programs(self) -> bool:
        """Check if the cemetery has any programs."""
        return len(self.cemetery) > 0

    def sample_from_cemetery(self) -> Optional[Program]:
        """
        Sample a program from the cemetery using a two-tier weighted strategy.
        1. It prioritizes programs that have been sampled the least.
        2. Among those, it favors older programs (earlier iteration_found).

        Returns:
            A program from the cemetery or None if it's empty.
        """
        if not self.has_cemetery_programs():
            return None

        # Get all valid programs and their stats from the cemetery
        candidates = []
        for pid, sample_count in self.cemetery.items():
            program = self.get(pid)
            if program:
                candidates.append({"program": program, "sample_count": sample_count})
        
        if not candidates:
            logger.warning("Cemetery has IDs but no valid, loadable programs found.")
            return None

        # Tier 1: Find the minimum sample count and filter for those candidates
        min_sample_count = min(c["sample_count"] for c in candidates)
        least_sampled_candidates = [
            c for c in candidates if c["sample_count"] == min_sample_count
        ]

        # Tier 2: Among the least sampled, apply weighting based on age
        programs_to_sample = [c["program"] for c in least_sampled_candidates]
        
        # Calculate weights. Older programs (smaller iteration_found) get higher weights.
        # The age is calculated relative to the most recent iteration to keep it relevant.
        weights = [
            float(self.last_iteration - p.iteration_found + 1) for p in programs_to_sample
        ]

        if not programs_to_sample:
            # This should not happen if candidates is not empty, but as a fallback:
            return None

        # Perform the weighted random sampling
        selected_program = random.choices(programs_to_sample, weights=weights, k=1)[0]

        # Increment the sample count for the selected program
        self.cemetery[selected_program.id] += 1
        
        logger.debug(
            f"Sampled program {selected_program.id} from cemetery "
            f"(sample count was {min_sample_count}, age-weight {self.last_iteration - selected_program.iteration_found + 1})"
        )
    ...
    def save(self, path: Optional[str] = None, iteration: int = 0) -> None:
        ...
        metadata = {
            ...
            "cemetery": list(self.cemetery.items()),
    ...
    def load(self, path: str) -> None:
        ...
        if os.path.exists(metadata_path):
            ...
            # Load cemetery with backward compatibility
            saved_cemetery = metadata.get("cemetery", [])
            if saved_cemetery and isinstance(saved_cemetery[0], str):
                # Old format: List[str] -> convert to OrderedDict with default count 0
                self.cemetery = OrderedDict((pid, 0) for pid in saved_cemetery)
            elif saved_cemetery and isinstance(saved_cemetery[0], list):
                # New format: List[Tuple[str, int]]
                self.cemetery = OrderedDict(saved_cemetery)
            else:
                # Empty or unknown format
                self.cemetery = OrderedDict()
    ...
    def _sample_parent(self) -> Program:
        ...
        """
        # --- New logic: Allow sampling from the cemetery of discarded programs ---
        cemetery_weight = getattr(self.config, "cemetery_sampling_weight", 0.0)
        if cemetery_weight > 0 and random.random() < cemetery_weight:
            cemetery_parent = self.sample_from_cemetery()
            if cemetery_parent:
                logger.info(f"Sampled parent {cemetery_parent.id} from cemetery for novelty.")
                # Store source information in metadata for debugging
                cemetery_parent.metadata["parent_source"] = "cemetery"
                return cemetery_parent
            else:
                logger.debug("Attempted to sample from cemetery, but it was empty or invalid.")
        # --- End of new logic ---
        ...
        if rand_val < self.config.exploration_ratio:
            ...
            parent.metadata["parent_source"] = "exploration"
            return parent
    ...
    def _enforce_population_limit(self, exclude_program_id: Optional[str] = None) -> None:
        ...
        for program in programs_to_remove:
            ...
            # Add the program to the cemetery before deleting
            self.add_to_cemetery(program)
