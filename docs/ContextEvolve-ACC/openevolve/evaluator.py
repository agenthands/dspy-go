...
class Evaluator:
    ...
    async def _cascade_evaluate(
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("evaluation_module", self.evaluation_file)
            if spec is None or spec.loader is None:
                logger.debug("Failed to load evaluation module spec, falling back to direct evaluation")
                return await self._direct_evaluate(program_path)

            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)

            # Check if cascade functions exist - if not, silently fall back to direct evaluation
            if not hasattr(module, "evaluate_stage1"):
                logger.debug("Cascade functions not found in evaluator, falling back to direct evaluation")
                return await self._direct_evaluate(program_path)
            ...
            # If cascade functions don't exist, fall back to direct evaluation silently
            # Otherwise, log the error
            if "evaluate_stage1" in str(e) or not hasattr(module, "evaluate_stage1"):
                logger.debug(f"Cascade evaluation not available, falling back to direct evaluation: {str(e)}")
                return await self._direct_evaluate(program_path)
            else:
