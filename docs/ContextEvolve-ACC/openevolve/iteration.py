...
async def run_iteration_with_shared_db(
    ...
    try:
        ...
        num_inspirations = (
            getattr(config.prompt, 'num_inspiration_programs', None) or 
            getattr(config.prompt, 'num_inspirations', None) or 
            getattr(config.database, 'num_inspirations', 5)
        )
        parent, inspirations = database.sample(n=num_inspirations)
        ...
        # Use config parameters for number of programs to retrieve
        # We need enough top programs to cover num_top_programs + num_diverse_programs
        num_top_needed = config.prompt.num_top_programs + config.prompt.num_diverse_programs
        island_top_programs = database.get_top_programs(num_top_needed, island_idx=parent_island)
        island_previous_programs = database.get_top_programs(config.prompt.num_previous_programs, island_idx=parent_island)
