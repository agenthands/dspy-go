...
// It implements the ExecutionEnvironment interface.
...
// Ensure YaegiREPL implements ExecutionEnvironment.
var _ ExecutionEnvironment = (*YaegiREPL)(nil)
...
// *ExecutionResult is now defined in env.go
...
func (r *YaegiREPL) injectBuiltins() error {
	symbols := interp.Exports{
		"rlm/rlm": {
			...
			"SHOW_VARS":    reflect.ValueOf(r.showVars),
			// Paper-native aliases
			"llm_query":         reflect.ValueOf(r.llmQueryRaw),
			"llm_query_batched": reflect.ValueOf(r.llmQueryBatchedRaw),
		},
	}
...
// The function will be available in the REPL directly (dot-imported).
// Uses a separate package path from builtins to avoid Yaegi's "constant definition loop" bug
// when calling interp.Use() multiple times on the same package.
func (r *YaegiREPL) RegisterFunction(name string, fn any) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.customToolNames = append(r.customToolNames, name)

	// Use a separate package path for custom tools to avoid collisions
	// with the "rlm/rlm" package used by builtins.
	symbols := interp.Exports{
		"rlmtools/rlmtools": {
	...
	// Dot-import to make the function available without qualification
	_, err := r.interp.Eval(`import . "rlmtools/rlmtools"`)
	return err
}
...
func (r *YaegiREPL) LoadContext(payload any) error {
	...
	case CodeContext:
		contentStr = v.Code
		_, err := r.interp.Eval(`var context = ` + strconv.Quote(v.Code))
		if err != nil {
			return err
		}
	case *CodeContext:
...
func (r *YaegiREPL) Execute(ctx context.Context, code string) (ExecutionResult, error) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			// In case of panic, we can't easily return named params if we change signature,
			// so we just log to stderr. The panic will propagate if we don't catch it,
			// or we can swallow it and return an error.
			// Ideally we'd modify the return values, but in a defer with named returns gone,
			// we'd need to use a pointer or closure capture if we wanted to change the return.
			// For now, we'll just log it to the buffer which will be picked up by the caller
			// if we return successfully, or we rely on the error.

			// Actually, to modify return values in defer without named returns is hard.
			// Let's keep it simple: we just don't crash.
			fmt.Fprintf(r.stderr, "interpreter panic: %v\n", rec)
		}
	}()
...
// GetState returns a string representation of the current environment state (variables/fact) for the LLM.
func (r *YaegiREPL) GetState() string {
	// Use GetVariableMetadata to get rich info
	vars := r.GetVariableMetadata()
	if len(vars) == 0 {
	...
	for _, v := range vars {
		...
		valStr := v.Preview
		if v.Length >= 0 {
			valStr = fmt.Sprintf("%s (len=%d)", valStr, v.Length)
		}
		fmt.Fprintf(&sb, "- %s (%s): %s\n", v.Name, v.Type, valStr)
	}
	return sb.String()
}
...
// GetContextInfo returns metadata about the loaded context.
// Aligned with reference: provides context type, total length, and chunk info.
func (r *YaegiREPL) GetContextInfo() string {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	switch ctx := v.Interface().(type) {
	case string:
		// Provide richer metadata like the reference implementation's QueryMetadata
		info := fmt.Sprintf("Your context is a string with %d total characters", len(ctx))
		if r.contextIndex != nil {
			chunkCount := r.contextIndex.ChunkCount()
			info += fmt.Sprintf(", broken into %d chunks", chunkCount)
		}
		if len(r.customToolNames) > 0 {
			info += fmt.Sprintf(". Available tools: %s", strings.Join(r.customToolNames, ", "))
		}
		return info
	default:
		info := fmt.Sprintf("Your context is a %T", ctx)
		if len(r.customToolNames) > 0 {
			info += fmt.Sprintf(". Available tools: %s", strings.Join(r.customToolNames, ", "))
		}
		return info
...
// GetVariableNames returns the names of user-defined variables in the REPL.
// This mirrors the reference implementation's format_execution_result which includes
// "REPL variables: [list]" so the LLM knows what state is available.
func (r *YaegiREPL) GetVariableNames() []string {
	r.mu.Lock()
	defer r.mu.Unlock()

	varNames := []string{
		"result", "answer", "data", "output", "response",
		"analysis", "summary", "final_answer", "count", "total",
		"items", "records", "values", "results", "findings",
		"subrlm_result", "chunk", "chunks", "prompts", "buffer",
	}
...
// Aligned with reference: includes stdout, stderr, and is formatted for LLM consumption.
...
// FormatExecutionResultWithVars formats execution result including REPL variable list.
// This matches the reference implementation's format_execution_result which appends
// "REPL variables: [list]" to help the LLM track available state.
func FormatExecutionResultWithVars(result ExecutionResult, varNames []string) string {
	base := FormatExecutionResult(result)
	if len(varNames) > 0 {
		base += fmt.Sprintf("\nREPL variables: %v", varNames)
	}
	return base
}
...
// showVars is the REPL builtin for listing available variables.
// It probes a set of common names since Yaegi doesn't provide an easy way to list all.
func (r *YaegiREPL) showVars() string {
	varNames := []string{
		"context", "result", "answer", "data", "output", "response",
		"analysis", "summary", "final_answer", "count", "total",
		"items", "records", "values", "results", "findings",
	...
	if len(found) == 0 {
		return "No variables created yet. Use assignments to create variables."
	}
	return "Available variables: " + strings.Join(found, ", ")
}
